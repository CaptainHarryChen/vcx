...
namespace VCX::Labs::Common {
    void OrbitCameraManager::Reset(Engine::Camera & camera) {
        Engine::Spherical sp0  = _position0 - camera.Target;
    ...
    void OrbitCameraManager::ProcessInput(Engine::Camera & camera, ImVec2 const & mousePos) {
        ...
        bool panningByKeyBoard[4] = {
            ImGui::IsKeyPressed(ImGuiKey_W) || ImGui::IsKeyPressed(ImGuiKey_UpArrow),
        ...
        if (panningByMouse || (EnablePan && (panningByKeyBoard[0] || panningByKeyBoard[1] || panningByKeyBoard[2] || panningByKeyBoard[3]))) {
            // perspective
            ...
            if (panningByKeyBoard[0]) panY -= 0.04f;
            if (panningByKeyBoard[1]) panY += 0.04f;
            ...
            if (ScreenSpacePanning) {
                _panOffset += q * glm::vec3(-panLeft, panUp, 0.f);
            } else {
                _panOffset += q * glm::vec3(-panLeft, 0.f, 0.f) + panUp * camera.Up;
            }
            _state |= StatePan;
        }
        if (rotating) {
